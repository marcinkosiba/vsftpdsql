...
static void try_change_to_homedir(struct vsf_session* p_sess);

void
process_post_login(struct vsf_session* p_sess)
{  
  str_getcwd(&p_sess->home_str);
  try_change_to_homedir(p_sess);
  ...
  while(1)
  {
    ...
    else if (str_equal_text(&p_sess->ftp_cmd_str, "QUIT"))
    {
      ...
      if (tunable_plugin_architecture)
      {
        struct Event_CliEnd event;
        gettimeofday(&event.timestamp,0);
        send_event_cliEnd(&event);
      }
      vsf_sysutil_exit(0);
    }
    ...
    else if ((!p_sess->is_anonymous || tunable_plugin_architecture) &&
...
{
  int retval;
  struct mystr cwd_pre_chdir = INIT_MYSTR;
  ...
  if (retval == 0)
  {
    ...
    {      
      ...
      request_result = vsf_request_read_directory(p_sess, &cwd_pre_chdir, &p_sess->ftp_arg_str);
      if (request_result.result)
      {
        str_chdir(&cwd_pre_chdir);
        vsf_cmdio_write(p_sess, FTP_FILEFAIL, request_result.reason);
...
handle_retr(struct vsf_session* p_sess)
{
  ...
  if (tunable_plugin_architecture)
  {
    ...
    str_getcwd(&dirPath);
    request_result= vsf_request_read_file(p_sess, &dirPath, &p_sess->ftp_arg_str);
    str_free(&dirPath);
    if (request_result.result)
    {
      vsf_cmdio_write(p_sess, FTP_FILEFAIL, request_result.reason);
      send_log_event_download_failed(p_sess);
      doneWith_Result_And_Reason(&request_result);
      return;
    }
    doneWith_Result_And_Reason(&request_result);
  ...
  if (tunable_plugin_architecture)
  {
    send_log_event_download(p_sess, &trans_ret);
  }
...
handle_dir_common(struct vsf_session* p_sess, int full_details, int stat_cmd)
{
  ...
  if (p_dir)
  {
    ...
    {
      struct Request_Result_And_Reason request_result;
      struct mystr parentDir = INIT_MYSTR;
      str_getcwd(&parentDir);
      request_result = vsf_request_read_directory(p_sess, &parentDir, &s_dir_name_str);      
      str_free(&parentDir);      
      if (request_result.result)
      {
        vsf_cmdio_write(p_sess, FTP_NOPERM, request_result.reason);
        doneWith_Result_And_Reason(&request_result);
        goto dir_close_out;        
      }
      doneWith_Result_And_Reason(&request_result);
...
handle_upload_common(struct vsf_session* p_sess, int is_append, int is_unique)
{
  ...
  {
    int tmpFD = str_open(p_filename,kVSFSysUtilOpenReadOnly);
    ...
    {
      {
        ...
        str_getcwd(&dirPath);      
        str_split_char_reverse(&dirPath, &dirName, '/');
        if (str_isempty(&dirPath))
        {
          str_alloc_text(&dirPath, "/");
        }
        if (str_isempty(&dirName))
        {
          str_alloc_text(&dirName, "/");
        }
        request_result = vsf_request_write_directory(p_sess, &dirPath, &dirName);      
        str_free(&dirPath);
        str_free(&dirName);
        if (request_result.result)
        {
          vsf_cmdio_write(p_sess, FTP_NOPERM, request_result.reason);
          send_log_event_upload_failed(p_sess, p_filename, is_append, doesExist);
          doneWith_Result_And_Reason(&request_result);
          return;
        }
        doneWith_Result_And_Reason(&request_result);
  ...
  else
  {
    if (tunable_plugin_architecture)
    {
      struct vsf_sysutil_statbuf * statbuf;
      if (doesExist) {
        struct Request_Result_And_Reason request_result;
        struct mystr dirPath = INIT_MYSTR;
        str_getcwd(&dirPath);
        request_result = vsf_request_write_file(p_sess, &dirPath, p_filename);
        str_free(&dirPath);
        if (request_result.result)
        {
          vsf_cmdio_write(p_sess, FTP_NOPERM, request_result.reason);
  ...
  if (tunable_plugin_architecture)
  {
    send_log_event_upload(p_sess, &trans_ret, p_filename, is_append, doesExist);
  }
  if (trans_ret.retval == 0)
  {
    vsf_log_do_log(p_sess, 1);
...
handle_mkd(struct vsf_session* p_sess)
{
  ...
  if (tunable_plugin_architecture)
  {
    struct Request_Result_And_Reason request_result;
    struct mystr dirPath = INIT_MYSTR;
    struct mystr dirPName = INIT_MYSTR;
    struct mystr dirPPath = INIT_MYSTR;
    const char * toSanitize;
    str_copy(&dirPath, &p_sess->ftp_arg_str);
    prepend_path_to_filename(&dirPath);
    toSanitize = str_strdup(&dirPath);
    sanitize_path(&toSanitize);
    str_alloc_text(&dirPath, toSanitize);
    dirPName = getParentDirectoryNameStr(&dirPath);
    dirPPath = getParentDirectoryPathStr(&dirPath);        
    request_result = vsf_request_write_directory(p_sess, &dirPPath, &dirPName);
    str_free(&dirPath);
    str_free(&dirPName);
    str_free(&dirPPath);
    ...
    {
      vsf_cmdio_write(p_sess, FTP_FILEFAIL, request_result.reason);
      send_log_event_mkdir_failed(p_sess);
      doneWith_Result_And_Reason(&request_result);
      return;
    }
    doneWith_Result_And_Reason(&request_result);
  ...
  if (tunable_plugin_architecture)
  {
    send_log_event_mkdir(p_sess, retval);
  }
  if (retval != 0)
  {
    vsf_cmdio_write(p_sess, FTP_FILEFAIL,
...
handle_rmd(struct vsf_session* p_sess)
{
  ...
  if (tunable_plugin_architecture)
  {
    ...
    struct mystr dirPath = INIT_MYSTR;
    str_getcwd(&dirPath);    
    request_result = vsf_request_write_directory(p_sess, &dirPath ,&p_sess->ftp_arg_str);
    str_free(&dirPath);
    if (request_result.result)
    {
      vsf_cmdio_write(p_sess, FTP_FILEFAIL, request_result.reason);
      send_log_event_rmdir_failed(p_sess);
      doneWith_Result_And_Reason(&request_result);
      return;
    }
    doneWith_Result_And_Reason(&request_result);
  ...
  if (tunable_plugin_architecture)
  {
    send_log_event_rmdir(p_sess, retval);
  }
  if (retval != 0)
  {
    vsf_cmdio_write(p_sess, FTP_FILEFAIL,
...
handle_dele(struct vsf_session* p_sess)
{
  ...
  if (tunable_plugin_architecture)
  {
    struct Request_Result_And_Reason result;
    result = checkFileWriteAccess(p_sess, &p_sess->ftp_arg_str);
    if (result.result)
    {
      vsf_cmdio_write(p_sess, FTP_FILEFAIL, result.reason);
      send_log_event_delete_failed(p_sess);
      doneWith_Result_And_Reason(&result);
      return;
    }
    doneWith_Result_And_Reason(&result);
  }
  ...
  if (tunable_plugin_architecture)
  {
    send_log_event_delete(p_sess, retval);
  }
  if (retval != 0)
  {
...
handle_rnto(struct vsf_session* p_sess)
{
  ...
  {    
    ...
    {
      vsf_cmdio_write(p_sess, FTP_FILEFAIL, request_result.reason);
      send_log_event_rename_failed(p_sess);
      doneWith_Result_And_Reason(&request_result);
      return;
    }
    doneWith_Result_And_Reason(&request_result);
  ...
  if (tunable_plugin_architecture)
  {
    send_log_event_rename(p_sess, retval);
  }
...
handle_site_chmod(struct vsf_session* p_sess, struct mystr* p_arg_str)
{
  ...
  {    
    ...
    struct mystr dirPPath = INIT_MYSTR;
    const char * toSanitize;
    str_copy(&dirPath, &s_chmod_file_str);
    prepend_path_to_filename(&dirPath);
    toSanitize = str_strdup(&dirPath);
    sanitize_path(&toSanitize);
    str_alloc_text(&dirPath, toSanitize);
  ...
  if (tunable_plugin_architecture)
  {
    send_log_event_chmod(p_sess, p_arg_str, &s_chmod_file_str, retval);
  }
...
void try_change_to_homedir(struct vsf_session* p_sess) {
  struct Request_Result_Homedir homedir = vsf_request_user_homedir(p_sess);
  struct mystr home = INIT_MYSTR;
  str_alloc_text(&home, homedir.homedir);
  str_chdir(&home);
}
