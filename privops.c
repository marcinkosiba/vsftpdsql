                    ...
                    const struct mystr* p_pass_str)
{
  ...
  if (result == kVSFLoginFail)
  {
    ...
    {
      struct Event_FailedLogin event;
      populate_shared_event_for_connection(&event.sharedInfo, p_sess);
      event.failed_password = str_strdup(p_pass_str);
      event.sharedInfo.user_name = str_strdup(&p_sess->user_str);
      send_event_failedLogin(&event);
      vsf_sysutil_free((char*)event.failed_password);
  ...
  else
  {
    ...
    {
      struct Event_Login event;
      populate_shared_event(&event.sharedInfo, p_sess);
      event.is_anonymous = p_sess->is_anonymous;      
      send_event_login(&event);
      vsf_sysutil_free((char*)event.sharedInfo.user_name);
    }
             ...
             const struct mystr* p_pass_str)
{
  ...
  {
    struct Request_Login request;
    struct Request_Result request_result;

    populate_shared_request_for_connection(&request.sharedInfo, p_sess);
    request.login = str_strdup(p_user_str);
    request.password = str_strdup(p_pass_str);
    request_result = send_request_login(&request);
    vsf_sysutil_free((char*)request.login);
    vsf_sysutil_free((char*)request.password);    
    if (request_result.result)
    {
      return kVSFLoginFail;
    } else {
      str_copy(&p_sess->plugin_arch_user_str, p_user_str);
